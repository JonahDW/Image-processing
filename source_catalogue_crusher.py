...
def main():
    ...
    # read the fits catalogue
    catalog = Table.read(fits_tab)
    ...
    if reset_flag:
        print('Resetting the quality flag column')
        catalog['Quality_flag'] = 1

    # get the pre-selection flag
    Quality_Flag     = catalog['Quality_flag']
    pre_select       = Quality_Flag.astype(dtype=bool)

    info ={}

    if doprtcatainfo.upper() == 'BASIC' or doprtcatainfo.upper() == 'FULL':

        # determine preselection counts
        pre_excluded_sources = np.count_nonzero(np.invert(pre_select))

        # determine information of the catalouge
        #
        ...
        info['number_of_catalogued_sources'] = total_no_of_sources
    ...
    if len(doselecttype) > 0:
        ...
        if list(np.unique(list(catalog['S_Code']))).count(doselecttype.replace('-','')) > 0:
            ...
            if doselecttype.count('-'):
                doselecttype = doselecttype.replace('-','')
            ...
            print('NOTE: No source type ',doselecttype,' catalogued, use full list of sources.')
    ...
    if tot_selected_sources > 0:
        if len(outputfilename) > 0:
            ...
            #
            catalog['Quality_flag'] = new_Q_select
            catalog.write(outputfilename, overwrite=True)

            print('New Catalogue file has been written',outputfilename)
    ...
    if len(kvisoutputfilename) > 0:
        ...
        write_annotation(kvisoutputfilename,
                         catalog['RA'][total_selection],
                         catalog['DEC'][total_selection],
                         source_Bmaj=catalog[kvisprt[0]][total_selection],
                         source_Bmin=catalog[kvisprt[1]][total_selection],
                         source_PA=catalog[kvisprt[2]][total_selection],
                         source_ID=catalog['Source_id'][total_selection],
                         INFORMATIONLINE=' Used parameter '+str(kvisprt)+'\n'+'# \n'+'#  No Sources: '+str(tot_selected_sources),
                         COLOR=kviscolor,CATALOGUE_NAME=fits_tab)
...
def new_argument_parser():
    ...
    parser.add_option('--RESET_FLAG', dest='reset_flag', action='store_true',
                      help='Reset the Quality flag column prior to making a selection')
